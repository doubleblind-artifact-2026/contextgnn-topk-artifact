# Beyond Two-Tower, Back to Local: What Remains of ContextGNN in Top-K Recommendation

This repository accompanies the paper **Beyond Two-Tower, Back to Local: What Remains of ContextGNN in Top-K Recommendation**, available at: \<TODO: link>.
...
The code in this repository is based on a modified version of [`danielemalitesta/Rel-DeepLearning-RecSys`](https://github.com/danielemalitesta/Rel-DeepLearning-RecSys), which provides the porting of ContextGNN to the top-K recommendation setting.

That repository, in turn, derives from the original ContextGNN implementation released by Kumo: [`kumo-ai/ContextGNN`](https://github.com/kumo-ai/ContextGNN). Consequently, the implementation follows the PyTorch and PyTorch Geometric code path shared by both repositories, while using the Elliot framework through the top-K recommendation port.
...
The fixed-gate checkpoints are available from the GitHub release at <TODO: link>. Download `best_weights.zip` from the release assets, then place the checkpoint files in `results/weights/` before running the fixed-gate inference-only tests.
...
- replace `<TODO: name>` with the name of the repository directory;
- replace `<TODO: link>` with the repository URL;
- adapt the training configuration inside the notebook according to the dataset and experimental setting to be reproduced;
- replace `<TODO: dataset>` in the final training command with the desired dataset name, for example `amazon-book`.
