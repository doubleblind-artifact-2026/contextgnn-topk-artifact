...
class DataSetLoader(LoaderCoordinator):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        if config.data_config.strategy == "fixed":
            ...
            if not path_val_data:
                raise ValueError("validation_path is required for the fixed strategy in this pipeline.")
            ...
            implicit_data = bool(config.binarize) or all(self.train_dataframe["rating"].isna())
            ...
            self.tuple_list, self.side_information = self.coordinate_information(
                self.tuple_list,
...
class DataSet(AbstractDataset):
    ...
    def __init__(self, config, data_tuple, side_information_data, *args, **kwargs):
        ...
        self.allunrated_mask_val = np.where((self.sp_i_train.toarray() == 0), True, False)
        self.sp_i_train_val = self.sp_i_train
        self.allunrated_mask_test = self.allunrated_mask_val
        ...
        if len(data_tuple) == 2:
            ...
            if hasattr(config, "negative_sampling"):
                ...
                self.test_mask = self.candidate_dict_to_bool_mask(self.test_negative_dict)
        else:
            self.val_dict = self.build_dict(data_tuple[1], self.users)
            self.test_dict = self.build_dict(data_tuple[2], self.users)

            self.sp_i_val = self.to_bool_sparse(self.val_dict)
            self.sp_i_train_val = ((self.sp_i_train.astype(bool) + self.sp_i_val.astype(bool)) > 0).astype(bool)
            self.allunrated_mask_test = np.where((self.sp_i_train_val.toarray() == 0), True, False)
            ...
            if hasattr(config, "negative_sampling"):
                ...
                self.val_mask = self.candidate_dict_to_bool_mask(self.val_negative_dict)
                self.test_mask = self.candidate_dict_to_bool_mask(self.test_negative_dict)

        self.allunrated_mask = self.allunrated_mask_test
    ...
    def get_relbench_df(self):
        base_dir = os.path.dirname(self.config.data_config.train_path)

        src_df = pd.read_parquet(os.path.join(base_dir, 'src_df.tsv'))
        dst_df = pd.read_parquet(os.path.join(base_dir, 'dst_df.tsv'))
        target_df = pd.read_parquet(os.path.join(base_dir, 'target_df.tsv'))
        train_df = pd.read_parquet(os.path.join(base_dir, 'train_df.tsv'))
        val_df = pd.read_parquet(os.path.join(base_dir, 'val_df.tsv'))
        test_df = pd.read_parquet(os.path.join(base_dir, 'test_df.tsv'))
        ...
        val_df['user_id'] = val_df['user_id'].map(self.public_users)
        val_df['item_id'] = val_df['item_id'].map(lambda x: [self.public_items[el] for el in x])
        ...
        val_df['pseudo_time'] = pd.to_datetime(val_df['pseudo_time'], unit='s')
        test_df['pseudo_time'] = pd.to_datetime(test_df['pseudo_time'], unit='s')

        return src_df, dst_df, target_df, train_df, val_df, test_df
    ...
    def candidate_dict_to_bool_mask(self, candidate_dict):
        rows = []
        cols = []
        ...
        for user, items in candidate_dict.items():
            ...
            u = self.public_users[user]

            for item in items:
                if item in self.public_items:
                    rows.append(u)
                    cols.append(self.public_items[item])
