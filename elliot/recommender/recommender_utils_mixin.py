...
class RecMixin(object):
    ...
    def get_candidate_mask(self, validation=False):
        if self._negative_sampling:
            if validation:
                if not hasattr(self._data, "val_mask"):
                    raise AttributeError("Validation mask is missing.")
                
                return self._data.val_mask

            if not hasattr(self._data, "test_mask"):
                raise AttributeError("Test mask is missing.")
            
            return self._data.test_mask

        if validation:
            return getattr(self._data, "allunrated_mask_val", self._data.allunrated_mask)

        return getattr(self._data, "allunrated_mask_test", self._data.allunrated_mask)
    ...
    def get_results(self):
        best_idx = self.get_best_arg()
        best_results = copy.deepcopy(self._results[best_idx])

        final_test_results = getattr(self, "_final_test_results", None)

        if final_test_results is not None:
            for k, payload in final_test_results.items():
                best_results.setdefault(k, {})
                best_results[k]["test_results"] = payload.get("test_results", {})
                best_results[k]["test_statistical_results"] = payload.get("test_statistical_results", {})
