...
class Evaluator(object):
    ...
    def eval_validation(self, recommendations):
        return self._eval_split(recommendations, split="validation")

    def eval_test(self, recommendations):
        return self._eval_split(recommendations, split="test")

    def _eval_split(self, recommendations, split="validation"):
        result_dict = {}

        for k in self._k:
            if split == "validation":
                split_data = self._val if hasattr(self, "_val") else None
                eval_objs = self._val_evaluation_objects if hasattr(self, "_val_evaluation_objects") else None
                split_name = "Validation"
            elif split == "test":
                split_data = self._test if hasattr(self, "_test") else None
                eval_objs = self._evaluation_objects if hasattr(self, "_evaluation_objects") else None
                split_name = "Test"
            else:
            ...
            results, statistical_results = self._process_test_data(
                recommendations,
                split_data,
                eval_objs,
                split_name
            ...
            if split == "validation":
                result_dict[k] = {
                    "val_results": results or {},
                    "val_statistical_results": statistical_results or {},
                    "test_results": {},
                ...
                result_dict[k] = {
                    ...
                    "test_statistical_results": statistical_results or {}
                }
    ...
    def _compute_needed_recommendations(self):
        ...
        if full_recommendations_metrics:
            self.logger.warning("At least one basic metric requires full length recommendations")
        if full_recommendations_additional_metrics:
            self.logger.warning("At least one additional metric requires full length recommendations")
        if full_recommendations_metrics or full_recommendations_additional_metrics:
