...
def to_sparse_adj_t(
    ...
    if isinstance(edge_index, Tensor) and edge_index.dim() == 2 and edge_index.size(0) == 2 and edge_index.dtype == torch.long:
        ...
        return SparseTensor(row=dst, col=src, value=val, sparse_sizes=(num_dst, num_src)).coalesce()
    ...
    if isinstance(edge_index, Tensor) and edge_index.is_sparse:
        ...
        return SparseTensor(row=row, col=col, value=val, sparse_sizes=(num_dst, num_src)).coalesce()
    ...
    raise ValueError("edge_index format not supported for NGCFLayer.")
...
class NGCFLayer(torch.nn.Module):
    def __init__(self, in_dim: int, out_dim: int, aggr: str = "add"):
        super().__init__()

        self.lin1 = torch.nn.Linear(in_dim, out_dim, bias=True)
        self.lin2 = torch.nn.Linear(in_dim, out_dim, bias=True)
    ...
    def forward(
        ...
        side = edge_index.matmul(x_src)
        out = self.lin1(side) + self.lin2(side * x_dst)
